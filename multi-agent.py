...
AGENT1_ID = "asst_OU7xQjbxVDsCEGvuhFgZgxXW"

# Web Search Agent (Agent 2)
AGENT2_ID = "asst_tCbk505ygq8xNaZUgQOqHKs6"

# Summary Agent (Agent 3)
AGENT3_ID = "asst_1Vfj9fYdA2PK7OJwRcM2Kog2"
...
async def main():
    ...
    user_query = input("Ask anything related to Alibaba and 40 thieves story :")
...
# Retrieve pre-created agent definitions for each role
        ...
        response_doc = await document_agent.get_response(
            thread_id=thread1.id,
                        messages=user_query
        )
        ...
        response_web = await web_agent.get_response(
            thread_id=thread2.id,
            messages=["Web search results for '{user_query}':\n{web_results}"]
        )
        ...
        response_summary = await summary_agent.get_response(
            thread_id=thread3.id,
            messages=[combined_info]
        )
